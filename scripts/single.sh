...
while  [ ! -S /usr/local/var/run/redis/redis.sock ]; do
        sleep 1
done

echo "Testing redis status..."
X="$(redis-cli -s /usr/local/var/run/redis/redis.sock ping)"
while  [ "${X}" != "PONG" ]; do
...
if [ $PGFAIL -ne 0 ]; then
	...
	if [ $PGUPFAIL -ne 0 ]; then
		...
		echo " DB Upgrade was a success. Starting postgresql 14"
