...
def testAgentOpenVas_whenBinaryAvailable_RunScan(
    openvas_agent_no_scope, scan_message, mocker
):
    """Tests running the agent and parsing the json output."""
    star_scan_mocker = mocker.patch(
    ...
    with open("tests/openvas_result.csv", "r", encoding="UTF-8") as f:
        ...
        output = {
            ...
            "CVEs": "",
            "Task ID": "",
            "Task Name": "PRODUCT_TEST_ONLY",
            "Timestamp": "2018-03-21T10:19:16+08:00",
            "Result ID": "",
            "Impact": "",
            "Solution": "",
        ...
        vulnerability_location = vuln_utils.VulnerabilityLocation(
            metadata=[
                vuln_utils.VulnerabilityLocationMetadata(
...
def testAgentOpenVas_whenLinkAssetAndBinaryAvailable_RunScan(
    openvas_agent, scan_message_link, mocker
):
    """Tests running the agent and parsing the json output."""
...
def testAgentOpenVas_whenLinkAssetGiven_NotScan(
    openvas_agent, scan_message_link_2, mocker
):
    """Tests running the agent and parsing the json output."""
    mocker.patch("agent.openvas.OpenVas.start_scan", return_value="hduzehfuhehfuhef")
    mocker.patch("agent.openvas.OpenVas.wait_task", return_value=None)
...
def testAgentOpenVas_whenServiceAssetGiven_RunScan(
    openvas_agent, scan_message_service, mocker
):
    """Tests running the agent and parsing the json output."""
    mocker.patch("agent.openvas.OpenVas.start_scan", return_value="hduzehfuhehfuhef")
    mocker.patch("agent.openvas.OpenVas.wait_task", return_value=None)
...
def testAgentOpenVas_whenDomainNameAssetGiven_NotScan(
    openvas_agent, scan_message_domain_2, mocker
):
    """Tests running the agent and parsing the json output."""
    mocker.patch("agent.openvas.OpenVas.start_scan", return_value="hduzehfuhehfuhef")
    mocker.patch("agent.openvas.OpenVas.wait_task", return_value=None)
...
def testAgentOpenVas_whenBinaryAvailableAndRangeOfIPsIsInput_RunScan(
    openvas_agent_no_scope, ip_range_message, mocker
):
    """Tests running the agent and parsing the json output for an IP range."""
    star_scan_mocker = mocker.patch(
        "agent.openvas.OpenVas.start_scan", return_value="starting scan.."
    )
    mocker.patch("agent.openvas.OpenVas.wait_task", return_value=None)
